...
const OrbitalEcosystem = ({ hideHeader = false, className = "" }) => {
  // Use a mobile-optimized radius
  const radius = 240; 
  const totalCards = ecosystemNodes.length;

  return (
    <div className={`py-24 w-full h-[700px] lg:h-[900px] xl:h-[1000px] flex flex-col items-center justify-center relative bg-black ${className}`}>
      
      {!hideHeader && (
        <div className="absolute top-10 text-center z-50 px-6 w-full">
          <h2 className="text-3xl lg:text-5xl font-bold tracking-tight mb-2">Powering Every Industry</h2>
          <p className="text-white/60 lg:text-xl">One unified system. Every business vertical solved.</p>
        </div>
      )}

      {/* Scaled wrapper to ensure the orbit fits on mobile */}
      <div className="relative w-[320px] h-[320px] sm:w-[400px] sm:h-[400px] flex items-center justify-center scale-[0.6] sm:scale-75 lg:scale-[1.2] xl:scale-[1.4] translate-y-[20%] lg:translate-y-[15%]">
