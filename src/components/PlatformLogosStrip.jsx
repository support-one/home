  ...
  return (
    <section className="py-16 overflow-hidden">
      <div className="text-center mb-10 px-6">
        <p className="text-[10px] lg:text-sm uppercase tracking-[0.3em] font-bold text-white/40 mb-2">Powered by React, Node.js, Flutter</p>
        <h2 className="text-lg lg:text-3xl font-bold text-white tracking-widest uppercase lg:mb-4">Multi-Platform Support</h2>
        <p className="text-[9px] lg:text-base text-white/40 mt-2 tracking-widest">macOS • WINDOWS • iOS • ANDROID & MORE</p>
      </div>
      ...
      <div className="relative w-full overflow-hidden">
        ...
        >
          {doubled.map((p, i) => (
            <div key={i} className="flex flex-col items-center gap-2 lg:gap-4 opacity-60 hover:opacity-100 transition-opacity">
              <div className="scale-100 lg:scale-150">
                {p.svg}
              </div>
              <span className="text-[9px] lg:text-xs uppercase tracking-[0.15em] text-white/40 font-bold">{p.name}</span>
            </div>
          ))}
        </motion.div>
      </div>
